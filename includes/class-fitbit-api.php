...
class FitBitPHP
{
    ...
    protected $metric = 0;
    protected $userAgent = 'FitbitPHP 0.73';
    protected $debug;
    ...
    public function initSession($callbackUrl, $cookie = true)
    {
        ...
        if (empty($session)) {
            session_start();
				...
				error_log($session, 3, '/tmp/ryan-errors.log');
        if (empty($_SESSION['fitbit_Session']))
					error_log(', fitbit_Session was empty, setitng to 0', 3, '/tmp/ryan-errors.log');

        if (empty($_SESSION['fitbit_Session']))
            $_SESSION['fitbit_Session'] = 0;

        if (!isset($_GET['oauth_token']) && $_SESSION['fitbit_Session'] == 1)
					error_log(', had session 1 but no oauth token, setting session to 0', 3, '/tmp/ryan-errors.log');
        ...
        if ($_SESSION['fitbit_Session'] == 0) {
						error_log(', sesison was 0 ', 3, '/tmp/ryan-errors.log');
            ...
            $_SESSION['fitbit_Session'] = 1;
						error_log(', found secret: ' . $_SESSION['fitbit_Secret'] . " moving session to 1\n", 3, '/tmp/ryan-errors.log');

            header('Location: ' . $this->authUrl . '?oauth_token=' . $request_token_info['oauth_token']);
            exit;

        } else if ($_SESSION['fitbit_Session'] == 1) {
						error_log(', sesison was 1 ', 3, '/tmp/ryan-errors.log');

            if( !isset( $_GET['oauth_token'] ) ){
								error_log(', grabbing oauth token', 3, '/tmp/ryan-errors.log');
                $request_token_info = $this->oauth->getRequestToken($this->requestTokenUrl, $callbackUrl);
                $this->oauth->setToken($request_token_info['oauth_token'], $_SESSION['fitbit_Secret']);
            }else{
								error_log(', have oauth token', 3, '/tmp/ryan-errors.log');
    ...
    public function getTimeSeries($type, $basedate, $to_period)
    {
        ...
        if (!strcmp($responseInfo['http_code'], '200')) {
            ...
            throw new FitBitException($responseInfo['http_code'], 'Fitbit request failed. Code: ' . $responseInfo['http_code'] . var_export($responseInfo, true) . var_export($headers, true));
        }
    }
...
// Check that the class exists before trying to use it
if ( ! class_exists('FitBitException')) {
...
// Check that the class exists before trying to use it
if ( ! class_exists('FitBitResponse')) {
...
// Check that the class exists before trying to use it
if ( ! class_exists('FitBitRateLimiting')) {
